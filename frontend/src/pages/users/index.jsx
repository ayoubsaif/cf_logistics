...
const UsersPage = ({ siteConfig, stores, users, role, error, token }) => {
  siteConfig = {
    ...siteConfig,
    stores,
  }
    ...
    <>
      ...
      <Layout title="Usuarios" siteConfig={siteConfig}>
        <Flex mb={5} flexDirection={['column', 'row']} justifyContent='space-between' gap={4}>
          ...
          {role === 'admin' && (
            <Button colorScheme='brand' as={NextLink} href='/users/create'>
              <Icon as={AddIcon} boxSize={5} mr={3} />
              Crear Usuario
            </Button>
          )}
        </Flex>
        <UsersTable initialUsers={users} token={token}/>
      </Layout>
    </>
  );
};
